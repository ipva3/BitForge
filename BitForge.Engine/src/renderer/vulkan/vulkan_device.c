...
b8 vulkan_device_create(vulkan_context* context) {
  ...
  BFINFO("Creating logical device...");
...
void vulkan_device_destroy(vulkan_context* context) {
  ...
  BFINFO("Destroying logical device...");
  ...
  BFINFO("Releasing physical device resources...");
...
b8 select_physical_device(vulkan_context* context) {
  ...
  if (physical_device_count == 0) {
    BFFATAL("No devices which support Vulkan where found.")
  }
  ...
  for (u32 i = 0; i < physical_device_count; ++i) {
    ...
    if (result) {
      BFINFO("Selected device: '%s'", properties.deviceName);
      // GPU type, etc.
      switch (properties.deviceType) {
      default:
      case VK_PHYSICAL_DEVICE_TYPE_OTHER:
        BFINFO("GPU type is unknown.");
        break;
      case VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU:
        BFINFO("GPU type is Integrated.");
        break;
      case VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU:
        BFINFO("GPU type is Discrete.");
        break;
      case VK_PHYSICAL_DEVICE_TYPE_VIRTUAL_GPU:
        BFINFO("GPU type is Virtual.");
        break;
      case VK_PHYSICAL_DEVICE_TYPE_CPU:
        BFINFO("GPU type is CPU.");
        break;
      }
      ...
      for (u32 j = 0; j < memory.memoryHeapCount; ++j) {
        ...
        if (memory.memoryHeaps[j].flags & VK_MEMORY_HEAP_DEVICE_LOCAL_BIT) {
          BFINFO("Local GPU memory: %.2f GiB", memory_size_gib);
        } else {
          BFINFO("Shared System memory: %.2f GiB", memory_size_gib);
        }
      }
  ...
  if (!context->device.physical_device) {
    BFERROR("No physical devices were found which meet the requirements.");
    return false;
  }

  BFINFO("Physical device selected.");
  return true;
}
...
b8 physical_device_meets_requirements(
  ...
  if (requirements->discrete_gpu) {
    if (properties->deviceType != VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU) {
      BFINFO("Device is not a discrete GPU, and one is required. Skipping.");
      return false;
    }
  }
  ...
  BFINFO("Graphics | Present | Compute | Transfer | Name");
  ...
  for (u32 i = 0; i < queue_family_count; ++i) {
    ...
    if (queue_families[i].queueFlags & VK_QUEUE_TRANSFER_BIT) {
      // Take index if it is the current lowest. This increase the
  ...
  if (
    ...
    BFINFO("Device meets queue requirements.");
    BFTRACE("Graphics Family Index: %i", out_queue_info->graphics_family_index);
    BFTRACE("Present Family Index: %i", out_queue_info->present_family_index);
    BFTRACE("Compute Family Index: %i", out_queue_info->compute_family_index);
    BFTRACE("Transfer Family Index: %i", out_queue_info->transfer_family_index);
    ...
    if (out_swapchain_support->format_count < 1 || out_swapchain_support->present_mode_count < 1) {
      ...
      BFINFO("Required swapchain support not present, skipping device.");
      return false;
    }
    ...
    if (requirements->device_extension_names) {
      ...
      if (available_extension_count != 0) {
        ...
        for (u32 i = 0; i < required_extension_count; ++i) {
          ...
          if (!found) {
            BFINFO("Required extension not found: '%s', skipping device.", requirements->device_extension_names[i]);
    ...
    if (requirements->sampler_anisotropy && !features->samplerAnisotropy) {
      BFINFO("Device does not support samplerAnisotropy, skipping.");
      return false;
    }
