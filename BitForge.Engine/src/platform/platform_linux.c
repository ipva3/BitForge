...
b8 platform_startup(
  ...
  if (xcb_connection_has_error(state->connection)) {
    BFFATAL("Failed to connect to X server via XCB.");
    return FALSE;
  }
  ...
  if (stream_result <= 0) {
    BFFATAL("An error occurred when flusing the stream: %d", stream_result);
    return FALSE;
  }
...
b8 platform_pump_messages(platform_state* plat_state) {
  ...
  while (event != 0) {
    ...
    case XCB_CONFIGURE_NOTIFY: {
      ...
      // Fire the event. The appliaction layer should pick this up, but not handle it
...
b8 platform_create_vulkan_surface(platform_state* plat_state, vulkan_context* context) {
  ...
  if (result != VK_SUCCESS) {
    BFFATAL("Vulkan surface creation failed!");
    return false;
  }
